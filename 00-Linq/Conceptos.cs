...
// MÉTODOS DE EXTENSIÓN Y EXPRESIONES LAMBDA EN LINQ 
/*
        Son dos conceptos clave que permiten escribir consultas de una manera clara y concisa.

        Los métodos de extensión:
            Son métodos que se aplican a una instancia de un tipo de datos, 
            como una colección de objetos, y que devuelven una nueva instancia del mismo tipo de datos 
            con alguna transformación aplicada. 

        Las expresiones lambda:
            Son funciones anónimas que se utilizan para expresar predicados o transformaciones dentro de una consulta LINQ. 

            var resultados = miColeccion.Where(objeto => objeto.Propiedad == valor);
 */

// SINTAXIS LINQ 
/*
        La sintaxis de consulta:
            Utiliza palabras clave que se asemejan a la sintaxis de SQL para expresar la consulta de datos. 

            var resultados = from objeto in miLista
                             where objeto.Propiedad == valor
                             orderby objeto.OtraPropiedad
                             select objeto;

        La sintaxis de método:
            Utiliza métodos de extensión y expresiones lambda para expresar la consulta. 

            var resultados = miLista.Where(objeto => objeto.Propiedad == valor)
                                    .OrderBy(objeto => objeto.OtraPropiedad);
 */
